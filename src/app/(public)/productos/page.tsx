    ...
    <>
      <section className="bg-gradient-to-br from-gray-900 to-green-950 py-24 text-white text-center">
        <div className="max-w-2xl mx-auto px-6">
          <span className="inline-block text-xs font-bold uppercase tracking-widest text-green-300 bg-green-300/15 px-3 py-1 rounded-full mb-4">Catálogo completo</span>
          ...
          <div className="space-y-14">
            {categories.map(cat => (
              <div key={cat.id} id={cat.id}>
                <div className="flex items-center gap-4 mb-6 pb-4 border-b-2 border-green-50">
                  <div className="w-12 h-12 rounded-xl bg-green-50 flex items-center justify-center text-2xl">{cat.icon}</div>
                  <h2 className="text-xl font-black text-gray-900">{cat.title}</h2>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {cat.items.map(item => (
                    <div key={item} className="flex items-center gap-2.5 bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 font-medium hover:border-[#15803d] hover:bg-green-50 transition-colors">
      ...
      <section className="bg-gradient-to-br from-[#166534] to-[#15803d] py-20">
        <div className="max-w-[1200px] mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <p className="text-green-100 text-lg">Cuéntanos tu proyecto y te damos precio sin compromiso.</p>
          </div>
          <div className="flex flex-wrap gap-4">
            <a href="/contacto" className="px-7 py-3.5 bg-white text-[#15803d] font-semibold rounded-lg hover:bg-gray-100 transition-colors">Solicitar presupuesto</a>
